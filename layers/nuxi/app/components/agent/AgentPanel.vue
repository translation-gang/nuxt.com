...
<template>
  <AgentPanelShell v-model:open="isOpen" :docked="isAgentDockedBreakpoint">
    ...
    <template #actions>
      <UTooltip text="Новый чат" :kbds="loggedIn ? ['meta', 'O'] : undefined">
        <UButton icon="i-lucide-plus" color="neutral" variant="ghost" aria-label="Новый чат" @click="startNewChat" />
      </UTooltip>
      <UTooltip text="Открыть на весь экран">
    ...
    <template #close>
      <UTooltip text="Закрыть" :kbds="['meta', 'i']">
        <UButton icon="i-lucide-x" color="neutral" variant="ghost" aria-label="Закрыть" @click="isOpen = false" />
      </UTooltip>
    </template>
