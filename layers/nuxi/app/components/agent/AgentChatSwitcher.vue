...
const matchingChats = computed(() => {
  ...
  return allChats.value.filter(c => (c.title || 'Без названия').toLowerCase().includes(q))
})
...
<template>
  ...
  >
    <UButton
      :label="activeChat ? (activeChat.title || 'Без названия') : 'Новый чат'"
    ...
    <template #content>
      ...
      <div v-if="filteredChats.length" class="flex flex-col py-1">
        ...
        >
          <span class="truncate min-w-0">{{ chat.title || 'Без названия' }}</span>
        ...
        <p v-if="hasMore" class="px-3.5 py-1.5 text-xs text-muted">
          +{{ matchingChats.length - PAGE_SIZE }} ещё — используйте поиск для фильтрации
        </p>
      </div>
