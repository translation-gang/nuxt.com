...
export default defineEventHandler(async (event) => {
  ...
  if (!chat || chat.userId !== user.id) {
    throw createError({ message: 'Доступ запрещён', status: 403, why: 'You do not own this chat.' })
  }
  ...
  if (!apiKey || !teamId || !projectId) {
    throw createError({ message: 'Интеграция с Linear не настроена', status: 503, why: 'The NUXT_LINEAR_API_KEY, NUXT_LINEAR_TEAM_ID, or NUXT_LINEAR_PROJECT_ID environment variables are missing.' })
  }
  ...
  if (!issue?.url) {
    throw createError({ message: 'Не удалось создать задачу в Linear', status: 500, why: 'The Linear API did not return an issue URL.' })
  }
