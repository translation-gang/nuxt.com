---
title: Nuxt UI v4
description: "Nuxt UI v4 объединяет Nuxt UI и Nuxt UI Pro в одну бесплатную библиотеку: 110+ компонентов, 12 шаблонов и Figma kit."
image: /assets/blog/nuxt-ui-v4.png
authors:
  - name: Benjamin Canac
    avatar:
      src: https://github.com/benjamincanac.png
    to: https://x.com/benjamincanac
  - name: Sébastien Chopin
    avatar:
      src: https://github.com/atinux.png
    to: https://x.com/atinux
  - name: Hugo Richard
    avatar:
      src: https://github.com/hugorcd.png
    to: https://x.com/hugorcd
date: 2025-09-22T10:00:00.000Z
category: Релиз
---

Мы выпускаем [**Nuxt UI v4**](https://ui.nuxt.com) — важный шаг и новый стандарт для нашей библиотеки компонентов. Nuxt UI и Nuxt UI Pro объединены в одну мощную и полностью бесплатную библиотеку с открытым исходным кодом.

Это открывает новую главу для экосистем Vue и Nuxt благодаря [переходу NuxtLabs в Vercel](https://nuxtlabs.com). Каждый компонент — от [простой кнопки](https://ui.nuxt.com/docs/components/button) до [боковой панели дашборда](https://ui.nuxt.com/docs/components/dashboard-sidebar) — теперь доступен всем.

То, что раньше было платным, доступно всем: более 100 компонентов, продвинутые секции и [готовые к продакшену шаблоны](https://ui.nuxt.com/templates) в одном месте.

## Собирайте что угодно быстрее

Nuxt UI v4 упрощает создание современных приложений: лендинги, страницы цен, документация, блоги, портфолио и т.д. без старта с нуля.

Всё необходимое в одном пакете `@nuxt/ui`:

- **110+ компонентов** — от простых сайтов до сложных приложений.
- **12 бесплатных шаблонов** — [лендинг](https://landing-template.nuxt.dev), [SaaS](https://saas-template.nuxt.dev), [дашборд](https://dashboard-template.nuxt.dev), [доки](https://docs-template.nuxt.dev), [портфолио](https://portfolio-template.nuxt.dev), [чат](https://chat-template.nuxt.dev), [changelog](https://changelog-template.nuxt.dev).
- **Контент и типографика** — Markdown и контентные сайты с prose-компонентами и Nuxt Content.
- **Vue и Nuxt** — работает в любом Vue- или Nuxt-проекте, а также с [Adonis](https://github.com/nuxt-ui-templates/starter-adonis) и [Laravel](https://github.com/nuxt-ui-templates/starter-laravel).

Весь набор Pro теперь общий: мощные компоненты, раньше только для платных пользователей, бесплатны для всех.

::tabs{class="gap-0"}

::div{label="index.vue"}
```vue
  <template>
    <UApp>
      <UHeader>
        <UNavigationMenu :items="navigation" />
        <template #right>
          <UColorModeButton />
          <UButton icon="i-simple-icons-github" />
        </template>
      </UHeader>

      <UPageHero
        title="Nuxt UI — стартовый шаблон"
        description="Nuxt UI — бесплатная UI-библиотека для приложений Nuxt. Создавайте красивые и отзывчивые приложения за минуты."
        :links="heroLinks"
      />

      <UPageSection
        title="Свобода создавать что угодно"
        description="Nuxt UI включает набор компонентов на все случаи. Меньше шаблонного кода без потери гибкости."
        :features="features"
      />

      <UPageSection title="Тарифы">
        <UPricingPlans :plans="plans" />
      </UPageSection>

      <UPageSection>
        <UPageCTA
          title="Начните с Nuxt UI уже сегодня!"
          description="Nuxt UI — бесплатная UI-библиотека с открытым исходным кодом для Nuxt."
          variant="subtle"
          :links="ctaLinks"
        />
      </UPageSection>

      <UFooter :items="footerItems" />
    </UApp>
  </template>
```
::

::code-preview
---
label: Превью
class: '[&>div]:*:my-0 w-full'
ui:
  preview: p-0
---
:index-example
::

::

## От макета к коду без разрывов

Успешный проект начинается с продуманной дизайн-системы. В v4 мы бесплатно выпускаем **полный Figma Kit**, повторяющий всю библиотеку компонентов.

Более **2000 вариантов компонентов и дизайн-токены** в одной точке входа во Figma, с пояснениями по структуре и использованию. Дизайнеры и разработчики работают из одного источника, коллаборация проще, дизайн и реализация совпадают.

:u-button{to="https://go.nuxt.com/figma-ui" target="_blank" icon="i-simple-icons-figma" label="Скачать Figma Kit →" class="mb-4"}

::carousel
---
items:
  - /assets/blog/figma-kit/1.png
  - /assets/blog/figma-kit/2.png
  - /assets/blog/figma-kit/3.png
---
::

## Улучшенный опыт разработки

Помимо новых компонентов, v4 приносит заметные улучшения в рабочий процесс.

### Простая миграция

В отличие от перехода с v2 на v3, миграция на v4 проста: фокус на объединении, а не на несовместимых изменениях. Большинство компонентов работают так же, существующий код в основном не трогаем.

Подробности — в [руководстве по миграции](https://ui.nuxt.com/getting-started/migration/v4).

### Обновлённая документация

Документация переработана: понятнее структура, сложные темы вынесены в отдельные страницы. Документация готова и для ИИ: **[MCP-сервер (Model Context Protocol)](https://ui.nuxt.com/docs/getting-started/ai/mcp)** даёт инструментам вроде Cursor прямой доступ к описанию компонентов и метаданным. Есть **[файлы `LLMs.txt`](https://ui.nuxt.com/docs/getting-started/ai/llms-txt)** — структурированный формат для любых ИИ-ассистентов. Вся библиотека доступна ИИ прямо в редакторе.

### Готовность к новой волне ИИ

Компоненты чата поддерживают **Vercel AI SDK v5**. Класс `Chat` и формат сообщений (с `parts`) совместимы с последними изменениями AI SDK.

## Благодарность пользователям Pro

Отдельное спасибо всем, кто поддерживал Nuxt UI Pro. Ваши раннее использование и обратная связь помогли сформировать Nuxt UI и довести проект до момента, когда эти возможности доступны всему сообществу.

Ваша поддержка сделала это возможным.

## Начните собирать уже сегодня

Создайте новый проект с любым из бесплатных шаблонов:

::code-group

```bash [Starter]
npm create nuxt@latest -- -t ui
```

```bash [Landing]
npm create nuxt@latest -- -t github:nuxt-ui-templates/landing
```

```bash [Docs]
npm create nuxt@latest -- -t github:nuxt-ui-templates/docs
```

```bash [SaaS]
npm create nuxt@latest -- -t github:nuxt-ui-templates/saas
```

```bash [Dashboard]
npm create nuxt@latest -- -t github:nuxt-ui-templates/dashboard
```

```bash [Chat]
npm create nuxt@latest -- -t github:nuxt-ui-templates/chat
```

```bash [Portfolio]
npm create nuxt@latest -- -t github:nuxt-ui-templates/portfolio
```

```bash [Changelog]
npm create nuxt@latest -- -t github:nuxt-ui-templates/changelog
```

::

Или добавьте в существующий проект:

```bash
npm install @nuxt/ui@latest
```

## Будущее — открытое

С единой кодовой базой и поддержкой Vercel мы как никогда готовы двигать границы разработки компонентов. Этот релиз не состоялся бы без сообщества и всех, кто помогал создавать Nuxt UI.

Ждём, что вы построите с Nuxt UI. Будущее UI-разработки — бесплатное, открытое и мощное.

---

*Начать просто: загляните в [документацию](https://ui.nuxt.com) и [Discord](https://discord.nuxt.com), чтобы общаться с разработчиками, которые собирают проекты на Nuxt UI.*
