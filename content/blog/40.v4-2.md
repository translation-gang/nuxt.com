...
Используйте только для тестов: включаются все будущие несовместимые изменения, в том числе переход на Nitro v3 после выхода интеграции.

::callout{type="warning"}
Функция экспериментальная, API может меняться. Включайте только если готовы к несовместимым изменениям и хотите повлиять на развитие Nuxt!
::
...
- **Переход к определению для динамических импортов** — переход к файлам при паттернах вида <code>import(\`~/assets/${name}.webp\`)</code>
- **Навигация по Nitro-маршрутам** — переход к серверным route handler'ам из `$fetch`, `useFetch`, `useLazyFetch`
- **Runtime config** — переход к определению для свойств runtime config
...
::warning
В VS Code нужно выбрать workspace-версию TypeScript: команда «TypeScript: Select TypeScript Version» → «Use Workspace Version» (используйте версию из workspace).
::
