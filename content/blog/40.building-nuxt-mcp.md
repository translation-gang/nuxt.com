...
- **Ресурсы (Resources)** — данные, которые сервер отдаёт как контекст для моделей: файлы, схемы БД, специфичная для приложения информация. Каждый ресурс идентифицируется URI.
- **Инструменты (Tools)** — возможность для моделей вызывать внешние системы и выполнять операции (поиск, вызовы API и т.д.).
- **Промпты (Prompts)** — переиспользуемые шаблоны промптов с аргументами, которые может вызывать пользователь.
...
### Инструменты: операции для моделей
...
### Промпты: переиспользуемые шаблоны
